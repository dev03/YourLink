...
static CGFloat kboardHeight = 25.0f;
...
-(void)keyboardWillShowOrHide:(NSNotification *)notification {
	...
	if ([self firstResponder:objectView]) {
		...
		heightforkeyboard:keyboardBounds.size.width];
	}
}
...
- (void)animateView:(BOOL)isShow textField:(id)textField heightforkeyboard:(CGFloat)kheight{
	...
	if (isShow) {
		if ([textField isKindOfClass:[UITextField class]]) {
            ...
            NSLog(@"%f %f",textPoint.x, textPoint.y);
            NSLog(@"%f", rect.size.height- textPoint.y- kheight);
            NSLog(@"%f", kheight);
